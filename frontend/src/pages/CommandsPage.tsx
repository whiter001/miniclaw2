...
export function CommandsPage() {
	...
	async function execute(request: CliRunRequest) {
		setLoading(true);
		setError(undefined);
		try {
			setResult(await runCli(request));
		} catch (runError) {
	...
	return (
		<div className="page-stack">
			<Card className="panel-card page-intro" bordered={false}>
				<Space direction="vertical" size="small">
					...
					<Typography.Paragraph type="secondary">
						这里放的是前端直接要用到的几类常见命令，避免每次都回到 Agent 页手输 prompt。
					...
					<Col xs={24} md={12} xl={8} key={item.key}>
						<Card className="panel-card preset-card" title={item.title}>
							<Space direction="vertical" size="middle" className="preset-card-body">
								...
								<Button type="primary" onClick={() => void execute(item.request)} loading={loading}>
									立即执行
								</Button>
							</Space>
						</Card>
			...
			<RunResultPanel loading={loading} error={error} result={result} emptyText="点击上面的快捷命令后，这里会显示结果。" />
		</div>
	);
}